...
class Bank {
    ...
    static async findAll() {
        try {
            const bank = await knex.select('id', 'name', 'code')
                .from('tb_bank')
    ...
    static async update(data) {
        try {
            ...
            return bank[0] ? { success: true, bank: bank[0] } : { success: false, message: 'Não foi possível atualizar o banco!' };
        } catch (error) {
            Message.warning(error);
    ...
    static async delete(id) {
        try {
            ...
            const existBank = await this.findOne(id);
            return existBank.success ? {success: false, message: 'Banco não deletado!'}:{ success: true, message: 'Banco deletado!' };
        } catch (error) {
            Message.warning(error);
