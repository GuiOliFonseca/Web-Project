...
class Material {
    ...
    static async findMostedUsed() {
        try {
            const material = await knex.select('tb_product.id_material', 'tb_material.name')
                .from('tb_product')
                .leftJoin('tb_material', 'tb_product.id_material', 'tb_material.id')
                .count('tb_product.id_material')
                .where({ "tb_product.is_deleted": false, 'is_active': true })
                .groupBy('tb_material.name')
                .groupBy('tb_product.id_material')
                .orderBy('count', 'desc')
                .limit(5)
