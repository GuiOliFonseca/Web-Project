...
class Client {
    static async findOne(id, ignore = false) {
        try {
            ...
            if (ignore) {
                client = await knex.select('id as id_client', 'id_user', 'is_deleted')
                    .from('tb_client')
                    .where({ id });
    ...
    static async retrieve(id_user, id_client) {
        try {
            return await knex.transaction(async trx => {
                ...
                return { success: true, message: 'Cliente recuperado!' };
            });
        } catch (error) {
            Message.warning(error);
            return { success: false, message: 'Cliente não recuperado!' };
        }
    }
    ...
    static async delete(id_user, id_client) {
        try {
            return await knex.transaction(async trx => {
                ...
                await trx('tb_address').update({ is_deleted: true }).where({ id_user });
