...
class Cart {
    ...
    static async findOne(id) {
        try {
            const cart = await knex.select([
                'tb_cart.id AS id_item', 
                'tb_cart.id_user', 'tb_salesman.business_name', 
    ...
    static async findCartByUser(id_user) {
        try {
    ...
    static async deleteAll(id_user) {
        try {
            await knex.delete()
                .table('tb_cart')
                .where({ id_user });

            return { success: true, message: 'Itens deletados!' };
        } catch (error) {
            Message.warning(error);
