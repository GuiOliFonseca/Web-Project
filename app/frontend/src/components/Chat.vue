<template>
  <div class="flex flex-col">
    <input type="checkbox" @click="start()" id="click" />
    <label class="bg-red-800" for="click">
      <i class="far fa-comments"></i>
    </label>
    <div class="wrapper mr-5">
      <div class="chat h-100">
        <div class="bg-gray-900 rounded-t-lg">
        ...
        <div v-if="!loading" class="messages h-100" id="messages">
          <template v-for="message in messages" :key="message.id">
            <div class="message" :class="{ me: message.posted_by == id_user }">
              {{ message.message }}
            </div>
            <small
              align="right"
        ...
        <div class="input">
          <input
            ...
            @keyup="sendMessage"
            class="rounded focus:border-red-800 focus:outline-none"
            type="text"
          />
          <i
            class="
...
export default {
  ...
  methods: {
    ...
    async start() {
      ...
      const res = await this.$socket.client.emit("identify", this.id_user);
