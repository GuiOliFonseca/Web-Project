      ...
      <div class="flex-none md:flex md:flex-row-reverse">
        <div
          class="
            ...
            md:w-4/6
            rounded-lg
            md:rounded-none md:rounded-tr-lg md:rounded-br-lg
            bg-gray-700
            pt-4
            pl-4
            pr-4
        ...
        >
          <div
            id="messages"
            v-if="chats.length"
            class="chat-area overflow-y-auto px-2"
          >
            <div
              ...
              :class="{
                'message-out rounded-bl-lg':
                  message.posted_by == $store.state.user.user.id_user,
                'message-in rounded-br-lg':
                  message.posted_by !== $store.state.user.user.id_user,
              }"
            >
              <span class="font-bold">{{
                message.posted_by == $store.state.user.user.id_user
                  ...
                  : chatOpen.other_user.business_name
              }}</span>
              <p>{{ message.message }}</p>
          ...
          <div v-if="!chats.length">
            <p class="text-gray-200 text-center sm:mt-5 sm:mb-5 lg:mt-20">
          ...
          <div v-if="chats.length" class="flex my-2">
            ...
            <button
              type="button"
              @click="sendMessage"
              class="mx-3 bg-red-800 text-white rounded-full h-10 w-16"
            >
        ...
        <div
          class="
            ...
            md:w-2/6
            rounded-lg
            md:rounded-none md:rounded-tl-lg md:rounded-bl-lg
            bg-gray-300
            p-4
          "
        >
          ...
          <div class="chat-area overflow-y-auto px-2">
            <div
              ...
              :class="{ 'bg-red-800 text-gray-200': chatOpen.index == index }"
            >
              <p
                class="truncate mr-4 my-auto align-middle"
                :class="{ 'text-gray-600': chatOpen.index != index }"
              >
...
export default {
  ...
  methods: {
    ...
    async openRoom(id_room, index) {
      ...
      this.chats.map((chat) => {
        if (chat.id_room == this.chatOpen.id_room) chat.read = true;
      });
      //console.log
      let messages = await Chat.retriveChatMessage(this.chatOpen.id_room);
      if (messages.success) {
    ...
    async sendMessage(event) {
      ...
      this.messages.push({
        message: msg,
        createdAt: new Date(),
        posted_by: this.$store.state.user.user.id_user,
      });
  ...
  sockets: {
    newMessage(data) {
      if (
        data.message.posted_by != this.$store.state.user.user.id_user &&
        this.chatOpen.id_room == data.id_room
      ) {
        this.messages.push(data.message);
        setTimeout(() => {
          this.scroll();
        }, 500);
      } else if(data.message.posted_by != this.$store.state.user.user.id_user){
        //console.log("de outra sala", data, this.chatOpen.id_room, this.chats);

        this.chats.map((chat) => {
          if (chat.id_room == data.id_room) chat.read = false;
        });
